...
Dans cette sections, nous détaillerons les éléments que nous avons identifiés comme remontables mais qui ont demandé des modifications pour être implémentés dans la classe \textbf{OWTextableBaseWidget}.

\subsubsection{Méthode \texttt{cancel()}}

\subsubsection{Méthode \texttt{manageGUIVisibility()}}

\subsubsection{Méthode \texttt{sendData()}}

\subsubsection{Méthode \texttt{task\_finished()}}

\subsection{Nouvelles méthodes et nouveaux attributs}

\subsubsection{Méthode \texttt{create\_widgetbox()}}

\subsubsection{Méthode \texttt{sendNoneToOutputs()}}
